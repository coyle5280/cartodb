...
cdb.admin.models.Filter = cdb.core.Model.extend({
  //Josh Note: Bucket limit histogram
...
cdb.admin.models.FilterDiscrete = cdb.core.Model.extend({
  //Josh Note: Bucket limit discrete
  DEFAULT_HIST_BUCKETS: 210,
  urlRoot: '/api/v1/filters',
...
var events = [
  ...
  _updateHist: function(hist) {
    //Assumption: This function uses the new hist data and compares it with the saved data so that any new values will also be included
    //But for remote filter we do not care about new values as we only show selected values when reached_limit is true.  
    //We do not limit the number of selectred values
  ...
  getSQLConditionForString: function() {
    ...
    if (!this.get("list_view") || this.get("column_type")) {
    ...
    if (items.length > 0) {
      ...
      // With the new remote filter lookup we might want to select all items and still have it filter the data set.
      //if reached_limit is true only otherwise the origin functionality should be executed  
