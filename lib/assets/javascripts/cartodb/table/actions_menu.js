  ...
  cdb.admin.LayersPanel = cdb.ui.common.TabPane.extend({
    ...
    initialize: function() {
      ...
      this.metadataPanelModel = new cdb.core.Model({ type: 'metadata' });
    ...
    *******/
    ...
    _resetLayers: function() {
      ...
      this._addLayers(this.map.layers);
      this._addLayer(this.metadataPanelModel);
    },
    ...
    _addLayer: function(layer, pos) {
      ...
      if (layer.get('type') == 'CartoDB' ||
        ...
        self._checkLayers();
      } else if (layer.get('type') === 'metadata') {
        var activePane = this.getActivePane();
        this.metadataPanelModel.set('table', activePane.table);
        this.metadataPanelModel.set('sqlView', activePane.sqlView);
        var v = new cdb.admin.MetadataPanelView({
          model: this.metadataPanelModel,
          map: self.map,
          vis: self.vis,
          user: self.user
        });
