...
function GrouperLayerMapView(mapViewClass) {
  ...
  return {
    ...
    initialize: function() {
      ...
      cdb.god.on('cancelDraw', this._cancelDraw, this);
    ...
    editGeometricFilter: function(layerId){
      // this.editFilterShapeControl.enable();
      this.featureGroups.forEach(function(item){
        if(item.id !== layerId){
    ...
    _createDrawControl: function(){
      ...
      // this.map_leaflet.addLayer(this.editableLayers);
  
    },

    _removeShapeFilter: function(layerId) {
      for(var i = 0; i < this.featureGroups.length; i++){
        if(this.featureGroups[i].id === layerId){
          this.map_leaflet.removeLayer(this.featureGroups[i].featureGroup);
          this.featureGroups.splice(i,1);
        }
      }
    },

    _showCorrectFilterShape: function(layerId, dataLayer){
      debugger
      this.activeShapeFilterId = layerId;
      this.featureGroups.forEach(function(item){
        if(item.id !== layerId){
          item.editControl.disable();
          item.active = false;
          this.map_leaflet.removeLayer(item.featureGroup);
        }else if(item.id === layerId){
          item.active = true;
          this.map_leaflet.addLayer(item.featureGroup);
          return;
        }
      }.bind(this));
      this._restoreShape(dataLayer);
    },

    _restoreShape: function(shapeData) {
      var layer = null;
      ...
      this.featureGroups.push({id: this.drawLayerId, featureGroup: editableLayers, editControl: editFilterShapeControl, active: true});
    ...
    _addGeometricToolbar: function() {
      ...
      //   // debugger
      //   // this.model.set('drawControl',new L.Control.Draw({
      //   draw: {
      //     polyline: false,
      //     polygon: {
      ...
      //   }
      // });
      // this.map_leaflet.addControl(this.drawControl);
      ...
      this.map_leaflet.on('draw:created', function(e){
        ...
        this.featureGroups.push({id: this.drawLayerId, featureGroup: editableLayers, editControl: editFilterShapeControl});
        this.drawEnabled = false;
        this.drawLayerId = null;
      }, this);
