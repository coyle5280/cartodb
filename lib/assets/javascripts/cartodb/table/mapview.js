...
function GrouperLayerMapView(mapViewClass) {
  ...
  return {
    ...
    initialize: function() {
      ...
      cdb.god.on('addGeometricFilter', this.addGeometricFilter, this);
      cdb.god.on('cancelDraw', this.cancelDraw, this);
      cdb.god.on('editDraw', this.editGeometricFilter, this);
      cdb.god.on('cancelDrawEdit', this.cancelEditGeometricFilter, this);
      cdb.god.on('updateMapShapeFilter', this._updateMapFilterShape, this);
      cdb.god.on('saveMapShapeFilter', this._saveGeometricFilter, this);
      cdb.god.on('removeShapeFilter', this._removeShapeFilter, this);
      cdb.god.on('setCircleRadius', this._setCircleFilterRadius, this);
    ...
    _getCircleFilterCenter: function(layerId){
      var coords = null;
      cdb.god.featureGroups.forEach(function(item){
        if(item.id === layerId){
          for(var l in item.featureGroup._layers){
            coords = item.featureGroup._layers[l]._latlng;
            break;
          }
        }
      }.bind(this));
    ...
    _setCircleFilterRadius: function(layerId, shapeType, newRadius){
      var layer = null;

      var circleCenter = this._getCircleFilterCenter(layerId);

      cdb.god.trigger('removeFilter', layerId);
      
      layer = L.circle([circleCenter.lat, circleCenter.lng], newRadius, {
         clickable: false,
          color: '#000',
          fillColor: '#ddd',
          weight: 2              
      ...
      this._ShapeFilterAdded(layerData);
      var editableLayers = new L.FeatureGroup();
      editableLayers.addLayer(layerData.layer);
      this.map_leaflet.addLayer(editableLayers);
      var editFilterShapeControl = new L.EditToolbar.Edit(this.map_leaflet, {
        featureGroup: editableLayers
      });
      if(cdb.god.featureGroups){
        this._deactivateFeatureGroups();
        cdb.god.featureGroups.push({id: layerId, featureGroup: editableLayers, editControl: editFilterShapeControl, active: true, visible: true});
      }else{
        cdb.god.featureGroups = [{id: layerId, featureGroup: editableLayers, editControl: editFilterShapeControl, active: true, visible: true}];
      }
    },

    editGeometricFilter: function(layerId, layerType){
      cdb.god.layerEditId = layerId;
      cdb.god.layerType = layerType;
      cdb.god.drawLayerId = layerId;
      cdb.god.featureGroups.forEach(function(item){
        if(item.id === layerId){
          if(item.editControl._map === null){
            item.editControl._map = this.map_leaflet;
          }
          this.leafletLayerId = item.leafletId;
          item.editControl.enable();
        }
      }.bind(this));
    },

    _saveGeometricFilter: function(layerId){
      cdb.god.featureGroups.forEach(function(item){
        if(item.id === layerId){
          if(item.editControl._enabled){
            //if map has been rebuilt since layer was added IE switching between map and table
            // update the map reference
            if(item.editControl._map === null){
              item.editControl._map = this.map_leaflet;
            }
            item.editControl.save();
            item.editControl.disable();
    ...
    cancelEditGeometricFilter: function(layerId){
      // this.layerEditId = null;
      cdb.god.layerEditId = null;
      cdb.god.layerType = null;
      cdb.god.featureGroups.forEach(function(item){
        if(item.id === layerId){
          if(item.editControl._map === null){
          }
          item.editControl.revertLayers();
          item.editControl.disable();
        }
      }.bind(this));
    },

    _disableAllGeoEdits: function(){
      if(cdb.god.featureGroups){
        cdb.god.featureGroups.forEach(function(item){
          if(item.editControl._enabled){
            item.editControl.disable();
    ...
    _updateMapFilterShape: function(layerId) {
      this._disableAllGeoEdits();
      if(!layerId){
        for(var i = 0; i < cdb.god.featureGroups.length; i++){
          // cdb.god.featureGroups[i].featureGroup._map.removeLayer(cdb.god.featureGroups[i].featureGroup);
          this.map_leaflet.removeLayer(cdb.god.featureGroups[i].featureGroup);
        }
      }else{
        for(var i = 0; i < cdb.god.featureGroups.length; i++){
          if(cdb.god.featureGroups[i].active === true && cdb.god.featureGroups[i].visible){
            cdb.god.featureGroups[i].featureGroup._map = this.map_leaflet;
            this.map_leaflet.addLayer(cdb.god.featureGroups[i].featureGroup);
          }else{

            // this.map_leaflet.removeLayer(cdb.god.featureGroups[i].featureGroup);
            if(cdb.god.featureGroups[i].featureGroup._map){
              cdb.god.featureGroups[i].featureGroup._map.removeLayer(cdb.god.featureGroups[i].featureGroup);
            }
    ...
    _removeShapeFilter: function(layerId) {
      for(var i = 0; i < cdb.god.featureGroups.length; i++){
        if(cdb.god.featureGroups[i].id === layerId){
          if(cdb.god.featureGroups[i].featureGroup._map){
              cdb.god.featureGroups[i].featureGroup._map.removeLayer(cdb.god.featureGroups[i].featureGroup);
            }
          // this.map_leaflet.removeLayer(cdb.god.featureGroups[i].featureGroup);
    ...
    addGeometricFilter: function(type, id){
      cdb.god.drawLayerId = id;
      ...
      switch(type) {
        ...
        case 'rec':
          ...
          this.drawObj = new L.Draw.Rectangle(this.map_leaflet, {
            metric: metric,
            shapeOptions:{
               clickable: false,
              color: '#000',
        ...
        case 'poly':
          ...
          this.drawObj = new L.Draw.Polygon(this.map_leaflet, {
            metric: metric,
            shapeOptions:{
               clickable: false,
              color: '#000',
        ...
        case 'cir':
          ...
          this.drawObj = new L.Draw.Circle(this.map_leaflet, {
              metric: metric,
              shapeOptions: {
                clickable: false,
                color: '#000',
    ...
    _deactivateFeatureGroups: function() {
      cdb.god.featureGroups.forEach(function(item){
        item.active = false;
      });
    },
    ...
    _addGeometricToolbar: function() {
      this.map_leaflet.on('draw:created', this._ShapeFilterAdded);
      this.map_leaflet.on('draw:created', function(e){
        var editableLayers = new L.FeatureGroup();
        ...
        if(cdb.god.featureGroups){
          this._deactivateFeatureGroups();
          cdb.god.featureGroups.push({id: cdb.god.drawLayerId, featureGroup: editableLayers, editControl: editFilterShapeControl, active: true, visible: true});
        }else{
          cdb.god.featureGroups = [{id: cdb.god.drawLayerId, featureGroup: editableLayers, editControl: editFilterShapeControl, active: true, visible: true}];
        }
        cdb.god.drawLayerId = null;
        
      }, this);

      this.map_leaflet.on('draw:edited', function(e){
        var layer = null;
        for(var i in e.layers._layers){
          layer = e.layers._layers[i];
          break;
        }
        if(layer){
          cdb.god.trigger('removeFilter', cdb.god.layerEditId);
          
          
          var layerData = {
            layer: layer,
            layerType: cdb.god.layerType,
    ...
    _ShapeFilterAdded: function(e) {
      if(e.layer){
        cdb.god.trigger('add:polygon_filter', e);
        cdb.god.trigger('updateFilterView', e);
      }
    },
...
cdb.admin.MapTab = cdb.core.View.extend({
  ...
  _addBindings: function() {
    ...
    this.map.layers.bind('change:visible', this._hideShowFilterShape, this);
  ...
  _hideShowFilterShape: function() {
    cdb.god.featureGroups.forEach(function(item){
      if(item.editControl._enabled){
        item.editControl.disable();
      }
      if(item.active && item.visible){
        this.mapView.map_leaflet.addLayer(item.featureGroup);
      }else{
        this.mapView.map_leaflet.removeLayer(item.featureGroup);
      }
    }.bind(this));
  },
  ...
  clearMap: function() {
    ...
    cdb.god.unbind("updateMapShapeFilter");
    cdb.god.unbind('addGeometricFilter');
    cdb.god.unbind('editDraw');
    cdb.god.unbind('cancelDrawEdit');
    cdb.god.unbind('updateMapShapeFilter');
    cdb.god.unbind('saveMapShapeFilter');
    cdb.god.unbind('removeShapeFilter');
    cdb.god.on('setCircleRadius');

    this.layerModel.trigger('cancelAllEdits');


    this.mapView._disableAllGeoEdits();
    if (this.mapView) {
      this.mapView.clean();
    }
  ...
  setupMap: function() {
    ...
    if (!this.map_enabled) {
      ...
      this._addOverlays();
      this._showPecan();
      if(cdb.god.featureGroups && cdb.god.featureGroups.length > 0){
        this._addActiveShape();
      }
  ...
  _addActiveShape: function() {
    this.mapView.map_leaflet._initPathRoot();
    this.mapView.map_leaflet._updatePathViewport();
    //These two functions are called because a bug in leaflet .7 does not set the pathViewport correctly
    cdb.god.featureGroups.forEach(function(item){
      //Need to update each featureGroup with a pointer to the new map
      item.featureGroup._map = this.mapView.map_leaflet;
      item.editControl._map = this.mapView.map_leaflet;
      //If layer was active restore
      if(item.active && item.visible){
          this.mapView.map_leaflet.addLayer(item.featureGroup);
      }
    }.bind(this));
