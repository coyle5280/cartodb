  ...
  cdb.admin.LayerPanelView = cdb.admin.RightMenu.extend({
    ...
    setDataLayer: function(dataLayer) {
      debugger
      if(dataLayer.get('filters') && dataLayer.get('filters').length > 0){
        var filters = dataLayer.get('filters');
        filters.forEach(function(item){
          if(item.column_type === 'geometry'){
            cdb.god.trigger('showFilterShapeForLayer', dataLayer.id, item.operations[0].metaData);
          }
        }.bind(this));
      }
