  ...
  cdb.admin.LayerPanelView = cdb.admin.RightMenu.extend({
    ...
    _addPolygonFilter: function(e) {
      ...
      // } else {
        filter = new cdb.admin.models.FilterDiscrete({
            ...
            operations: [{operation: "INTERSECTS"}],
            reached_limit: false,
            column: 'the_geom',
            list_view: false
          });
    ...
    _createShapeFilter: function(shapeData, id) {
      if(shapeData && shapeData.type && shapeData.layerData){
        ...
        if(cdb.god.featureGroups){
          cdb.god.featureGroups.push({id: id, featureGroup: editableLayers, editControl: editFilterShapeControl, active: true});
        }else{
          cdb.god.featureGroups = [{id: id, featureGroup: editableLayers, editControl: editFilterShapeControl, active: true}];
        }
      }
