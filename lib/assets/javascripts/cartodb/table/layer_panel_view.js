  ...
  cdb.admin.LayerPanelView = cdb.admin.RightMenu.extend({
    ...
    initialize: function() {
      ...
      this.model.bind('cancelAllEdits', this._clearAllShapeEdits, this);
    ...
    _clearAllShapeEdits: function(){
      this.filters.filters.forEach(function(item){
        if(item.get('column_type') === "geometry") {
          item.trigger('cancelAllEdits');
        }
      }.bind(this));
    },
    ...
    _switchVisibility: function(e) {
      ...
      this.setShapeFilterVisibility({id: this.model.id}, !this.model.get('visible'));
      this.updateShapeFilterEdit();
      this.model.save({ 'visible': !this.model.get('visible') });
      
    },


    updateShapeFilterEdit: function() {
      this.filters.filters.forEach(function(item){
        if(item.get('column_type') === "geometry"){
          item.trigger('editShapeActive');
        }
      }.bind(this));
    },

    //TODO Josh Add poly Filter
    _addPolygonFilter: function(e) {
      ...
      if (e.layerType == "circle") {
        ...
        layerArea = L.GeometryUtil.geodesicArea(e.layer._latlngs);
      }

      // var filter = (this.filters.filters) ? this.filters.filters.where({column: 'the_geom'}): {};
      // if (filter.length) {
      //   filter = filter[0]
      // } else {
        filter = new cdb.admin.models.FilterDiscrete({
            ...
            operations: [{
              ...
              metaData: {
                ...
                layerArea: layerArea,
                layerData: e.layer._latlng || e.layer._latlngs,
                type: e.layerType
              }
            }],
            reached_limit: false,
      ...
      var m = new cdb.admin.models.CategoricalOperation({
        ...
        text: JSON.stringify(correctLongCoords(shape.toGeoJSON().geometry))
      });
      filter.operations.reset([m]);

      //Because the map wraps we have more then 1 world so we need to adjust the coords to account for this
      function correctLongCoords(shapeGeom){
        var coords = shapeGeom.coordinates[0];
        for(var i = 0; i < coords.length; i++){
          while(coords[i][0] > 180 || coords[i][0] < -180){
            if(coords[i][0] > 180){
      ...
      function generateLayeData(layer) {
        var dataObj = {};
        if(true){
    ...
    _setLayerInfo: function(layer) {
      ...
      this.setView(layer);
      this.setShapeFilterVisibility(layer);
    },


    setShapeFilterVisibility: function(layerData, setting) {
      if(cdb.god.featureGroups){
        cdb.god.featureGroups.forEach(function(item){
          if(item.id === layerData.id) {
            if(layerData.get){
              item.visible = layerData.get('visible');
            }else{
    ...
    //TODO Josh Toggle button layer visibility
    ...
    _updateShapeFilters: function(data){
      var layerId = null;
      if(data.get('filters') && data.get('filters').length > 0){
        var filters = data.get('filters');
        for(var i = 0; i < filters.length; i++){
          if(filters[i].column_type === 'geometry') {
            layerId = data.id;
            if(!this._filterExists(data.id)){
              filters[i].operations[0].metaData.visible = data.get('visible');
              //Filter data only loads when panel expanded.  Add shape to featureGroups if not yet added
              this._createShapeFilter(filters[i].operations[0].metaData, data.id);
            }else{
              //Filter Shape has been added already just need to show it
              this._setActiveShapeFeatureGroup(data.id);
            }
          }
        }
      }else{
        //This layer Panel has no shape filter. Call with null will just remove the shape featureGroup from map 
        this._setActiveShapeFeatureGroup(null);
      }
      cdb.god.trigger('updateMapShapeFilter', layerId);
    },

    _filterExists: function(layerId){
      var result = false;
      if(cdb.god.featureGroups){
        cdb.god.featureGroups.forEach(function(item){
          if(item.id === layerId){
    ...
    _setActiveShapeFeatureGroup: function(layerId) {
      if(cdb.god.featureGroups){
        cdb.god.featureGroups.forEach(function(item){
          if(layerId && item.id === layerId){
            item.active = true;
          }else{
            item.active = false;
          }
    ...
    _createShapeFilter: function(shapeData, id) {
      if(shapeData && shapeData.type && shapeData.layerData){
        switch(shapeData.type){
          case "circle":
            layer = L.circle([shapeData.layerData.lat, shapeData.layerData.lng], shapeData.radius, {
                color: '#000',
                fillColor: '#ddd',
                weight: 2              
            });
          ...
          case "poly":
            var bounds = [];
            shapeData.layerData.forEach(function(item){
        ...
        //TODO Josh Do we need a map ref ??
        var editFilterShapeControl = new L.EditToolbar.Edit(null, {
            featureGroup: editableLayers
        });
        if(cdb.god.featureGroups){
          cdb.god.featureGroups.push({id: id, featureGroup: editableLayers, editControl: editFilterShapeControl, active: true, visible: shapeData.visible});
        }else{
          cdb.god.featureGroups = [{id: id, featureGroup: editableLayers, editControl: editFilterShapeControl, active: true, visible: shapeData.visible}];
        }
      }
    ...
    setDataLayer: function(dataLayer) {
      this._updateShapeFilters(dataLayer);
    ...
    setActiveWorkView: function(workView) {
      ...
      if(this.filters.filters){
        this.filters.filters.forEach(function(item){
          if(item.get('column_type') === 'geometry'){
            item.trigger('workView', workView);
          }
        }.bind(this));
      }
    },
    ...
    setActivePanelView: function(work_view) {
      ...
      cdb.god.workView = this.activeWorkView;
    },
