...
cdb.admin.MapOptionsDropdown = cdb.core.View.extend({
  ...
  _updateUnitsPref: function (value) {
    if (value.target.selectedIndex === 0){
      localStorage.setItem('units', 'english');
    } else {
  ...
  // _addUnitsCombo: function () {
  //   var className = '.unitsCombo';
  //   var element = this.$el.find('li.units');

  //   var combo = new cdb.forms.Combo({
  //     el: element,
  //     property: 'type',
  //     width: '90px',
  //     extra: ['English', 'Metric']
  //   });
  //   element.append(combo.render());
  ...
  render: function () {
    ...
    // this._addUnitsCombo();
    this.$el.find('.units').on('change', this._updateUnitsPref);
    if (localStorage.getItem('units')){
      this.$el.find('.units').val(localStorage.getItem('units'));
    }

    _.each(this.mapOptions.attributes, function (value, option) {
      var $el = this.$el.find('li.' + option);
      if ($el.find('a').hasClass('enabled')) {
        $el.addClass('active');
      } else {
