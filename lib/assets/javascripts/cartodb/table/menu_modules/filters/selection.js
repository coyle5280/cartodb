(function() {
  ...
  cdb.admin.mod.SelectorFilter = cdb.core.View.extend({
    ...
    events: {
      ...
      'click .drawRadious':                '_editCircleRadious'
    ...
    initialize: function() {
      ...
      this.radiousValDisplay = null;
      this.radiousVal = null;
      ...
      cdb.god.on('updateRadius', this.updateRadiousValue, this);
    },
    ...
    _editCircleRadious: function(e){
      e.preventDefault();
      e.stopPropagation();
      this.$circleRadiousText.hide();     
      this.$circleRadiousEdit.show(); 
      this.$circleRadiousEdit.focus();   
    },
    ...
    render: function() {
      ...
      if(this.model.get('operations') && this.model.get('operations')[0].operation === "INTERSECTS"){
        this.$el.html(this.getTemplate('table/menu_modules/filters/templates/shape_filter')({
          radiousValDisplay: this.radiousValDisplay,
          radiousVal: this.radiousVal
        }));
      }else{
      ...
      this.$circleRadiousText     = this.$el.find('.drawRadious');
      this.$circleRadiousEdit     = this.$el.find('.drawRadiousEdit');
    ...
    updateRadiousValue: function(e) {
      if(e.layer._mRadius){
        if(localStorage.getItem('units') === 'english'){
          this.radiousValDisplay = (e.layer._mRadius*0.000621371).toFixed(2) + "mi";
          this.radiousVal = (e.layer._mRadius*0.000621371).toFixed(2);
        }else{
          this.radiousValDisplay = (e.layer._mRadius/1000).toFixed(2) + "km";
          this.radiousVal = (e.layer._mRadius/1000).toFixed(2);
        }
      }
