(function() {
  ...
  var SelectorView = cdb.core.View.extend({
    ...
    initialize: function() {
      ...
      if(!this.options.reached_limit){
        ...
        if(this.model.get('userSelected')){
          var m = this.model.get('selected');
          delete this.model.attributes.userSelected
  ...
  cdb.admin.mod.SelectorFilter = cdb.core.View.extend({
    ...
    events: {
      ...
      'click a.sort':                 '_showSortDropdown'
    },
    ...
    _showSortDropdown: function(e) {
      this.killEvent(e);
      var container = $(e.target).parent().parent().parent().parent().parent().parent().parent();
      container.append(this.sortFilterDropdown.el);
      this.sortFilterDropdown.openAt(100, 65);
    ...
    _addItems: function() {
      ...
      if(this.sortFilterSelection === 'HighLowAZ'){
        this.sortArrayListHighLowAZ(this.model.items);
      }else {
        this.sortArrayListAZ(this.model.items);
      }
    ...
    render: function() {
      ...
      //Input to search and filter remote list reached Limit ipit
      this.$filter_remote.on('keyup', _.debounce(function(e){
        debugger
        this.model.items.add({bucket: "Texas", selected: false, value: "254", userSelected: true});
        this.model.items.add({bucket: "Kansas", selected: false, value: "105", userSelected: true});
        this.render();
      }.bind(this), 250));

      // this.$filter_input.on('keyup', function(){
      //   debugger
      //   this.$filter_input[this._togX(this.$filter_input.val())]('x');
      // }.bind(this)).on('mousemove', '.x', function( e ){
      //   this[_togX(this.offsetWidth-18 < e.clientX-this.getBoundingClientRect().left)]('onX');   
      // }.bind(this)).on('touchstart click', '.onX', function( ev ){
      //   ev.preventDefault();
