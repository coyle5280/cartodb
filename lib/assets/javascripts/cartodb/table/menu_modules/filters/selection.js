(function() {
  ...
  cdb.admin.mod.SelectorFilter = cdb.core.View.extend({
    ...
    events: {
      ...
      // 'keypress .filterInput':          '_filterList',
      'click a.apply':                     '_onApply',
      'click a.remove':                    '_remove',
      'click .reset':                      '_selectNone',
      'click .toggleButton':               '_toggleFilterView',
      ...
      'click .drawRadius':                '_editCircleRadius',
      'keypress .drawRadiusEdit':         '_editCircleRadiusKeyPress',
      'click .editDraw'                    : '_editDraw',
      'click .saveRadiusChange'            : '_updateCircleRadius',
      'click .cancelRadiusChange'          : '_cancelRadiusChange',
      'click .cancelShapeEdit'            : '_cancelShapeEdit',
      'click .saveShapeEdit'              : '_saveShapeEdit',
      'click  .drawRadiusEdit'            : '_clickEdit',
      'click .filterInput':                 '_filterInputClick',
      'click .filterInputRemote':            '_filterInputClick',
    ...
    initialize: function() {
      ...
      this.radiusValDisplay = null;
      this.radiusVal = null;
      this.layerId = this.options.layerId;
      this.drawShapeActive = false;
      this.drawShapeType = null;
      this.toggle = this.model.get('toggle');
      if(this.options.model.get('operations') && this.options.model.get('operations')[0] && this.options.model.get('operations')[0].metaData){
        var filterDataObj = this.options.model.get('operations')[0].metaData;
        this.drawShapeActive = true;
        this.drawShapeType = filterDataObj.type;
        if(filterDataObj.type === 'circle'){
          if(localStorage.getItem('units') === 'english') {
            this.radiusValDisplay = (filterDataObj.radius*0.000621371).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + " mi";
            this.radiusVal = (filterDataObj.radius*0.000621371).toFixed(2);
          } else {
            this.radiusValDisplay = (filterDataObj.radius/1000).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + " km";
            this.radiusVal = (filterDataObj.radius/1000).toFixed(2);
          }
        }else{
          if(localStorage.getItem('units') === 'english') {
            this.layerArea = (filterDataObj.layerArea*0.000621371).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + " mi";
          } else {
            this.layerArea = (filterDataObj.layerArea/1000).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + " km";
          } 
        }
      }

      if(cdb.god.workView === 'table'){
        this.tableView = true;
      }else{
      ...
      this.model.bind('editShapeActive', this._toggleEditFilterShape, this);
      this.model.bind('workView', this._setworkView, this);
      this.model.bind('cancelAllEdits', this._cancelAllEdits, this);

      this.add_related_model(this.model.items);
      this.add_related_model(this.model.operations);
      cdb.god.on('updateFilterView', this.updateFilterView, this);
      cdb.god.on('closeDialogs', this._saveRadiusChange, this);
      cdb.god.on('removeFilter', this._removeFilterChange, this);
      cdb.god.on('closeFilterDialogs', this._closeDialogs, this);
    },

    _setworkView: function (workview){
      if(workview === 'table'){
        this.tableView = true;
      }else{
        this.tableView = false;
    ...
    _filterInputClick: function (e){
      e.preventDefault();
      e.stopPropagation();
      cdb.god.trigger('closeFilterDialogs');
    },

    _closeDialogs: function (){
      if(this.sortFilterDropdown){
        this.sortFilterDropdown.hide();
      }
    },

    _cancelAllEdits: function () {
      this.editDrawStatus = false;
      this.radiusModeEdit = false;
      this.render();
    },
    ...
    _toggleEditFilterShape: function (){
      this.editDrawStatus = false;
      this.render();
    },


    _clickEdit: function (e){
      e.preventDefault();
      e.stopPropagation();
    },
    ...
    _cancelShapeEdit: function (e) {
      e.preventDefault();
      e.stopPropagation();
      this.editDrawStatus = false;
      cdb.god.trigger('cancelDrawEdit', this.layerId);
      this.render();
    },

    _removeFilterChange: function (id){
      var column = this.options.model.get('column');
      if(this.options.layerId === id && column === 'the_geom'){
        this._remove();
      }
    },

    _cancelRadiusChange: function (e){
      e.preventDefault();
      e.stopPropagation();
      this.radiusModeEdit = false;
      this.render();
    },
    _saveRadiusChange: function (e) {
      if(e){
        e.preventDefault();
        e.stopPropagation();
      }
      if(this.radiusModeEdit){
        this.radiusModeEdit = false;
        this.render();
      }
      if(cdb.god.currentFilterLi) {
    ...
    _saveShapeEdit: function (e) {
      if(e){
        e.preventDefault();
        e.stopPropagation();
      }
      this.editDrawStatus = false;
      cdb.god.trigger('saveMapShapeFilter', this.layerId);
      this.render();
    },

    _editDraw: function (e) {
      e.preventDefault();
      e.stopPropagation();
      this.editDrawStatus = true;
      cdb.god.trigger('editDraw', this.layerId, this.drawShapeType);
      this.render();
    },

    _editCircleRadius: function (e){
      e.preventDefault();
      e.stopPropagation();
      if(!this.editDrawStatus){
        if(!this.tableView){
          this.radiusModeEdit = true;
        }
      }
      this.render();
    },

    _editCircleRadiusKeyPress: function (e){
      if(e.keyCode === 13) {
        this._updateCircleRadius();
      }
    },

    _updateCircleRadius: function (e){
      if(e){
        e.preventDefault();
        e.stopPropagation();
      }     
      var newValue = this.$el.find('.drawRadiusEdit').val();
      if(localStorage.getItem('units') === 'english'){
        var correctedValue = (newValue*1609.34).toFixed(2);
      }else{
        var correctedValue = (newValue*1000).toFixed(2);
      }
      cdb.god.trigger('setCircleRadius', this.layerId, this.drawShapeType, correctedValue);
    },

    updateRadiusInput: function () {
      this.$circleRadiusText.show();     
      this.$circleRadiusEdit.hide();
      this.$saveCir.hide();
    },
    ...
    _updateCounter: function () {
      ...
      this.totalCount = (this.totalCount < 4999) ? this.totalCount : this.totalCount + "+"; 
    ...
    _showSortDropdown: function (e) {
      ...
      this.sortFilterDropdown.openAt(275, 100);
    ...
    render: function () {
      var self = this;
      // this.selectedCount = (this.model.get('count')) ? this.model.get('count').selected : 0;
      this.clearSubViews();
      var status = "loading";
      ...
      if(this.model.get('operations') && this.model.get('operations').length > 0 && this.model.get('operations')[0].operation === "INTERSECTS"){
        this.$el.html(this.getTemplate('table/menu_modules/filters/templates/shape_filter')({
          radiusValDisplay: this.radiusValDisplay,
          radiusVal: this.radiusVal,
          layerArea: this.layerArea,
          drawMode: this.drawShapeType,
          editDrawStatus: this.editDrawStatus,
          radiusModeEdit: this.radiusModeEdit,
      ...
      this.$circleRadiusText      = this.$el.find('.drawRadius');
      this.$circleRadiusEdit      = this.$el.find('.drawRadiusEdit');
      this.$saveCir               = this.$el.find('.saveRadiusChange');
      this.$CirImg                = this.$el.find('.circleImg');
      this.$PolyImg               = this.$el.find('.polyImg');
      this.$SqrImg                = this.$el.find('.squareImag');


      if(this.radiusModeEdit){
        this.$el.find('.drawRadiusEdit').focus();
      }
      ...
      if (this.$items.find("li").length > 4) {
        this.model.set("scrollers", true);
    ...
    updateFilterView: function (e) {
      this.drawShapeActive = true;
      this.drawShapeType = e.layerType;
      if(e.layer._mRadius){
        if(localStorage.getItem('units') === 'english'){
          this.radiusValDisplay = (e.layer._mRadius*0.000621371).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + "mi";
          this.radiusVal = (e.layer._mRadius*0.000621371).toFixed(2);
        }else{
          this.radiusValDisplay = (e.layer._mRadius/1000).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + "km";
          this.radiusVal = (e.layer._mRadius/1000).toFixed(2);
        }
      }else{
        if(localStorage.getItem('units') === 'english') {
          this.layerArea = (L.GeometryUtil.geodesicArea(e.layer._latlngs)*0.000621371).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + " mi";
        } else {
          this.layerArea = (L.GeometryUtil.geodesicArea(e.layer._latlngs)/1000).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}) + " km";
        }
      }
      this.render();
    },
    ...
    _remove: function (e) {
      ...
      if(this.tableView){
        this._updateFeatureGroup();
      }
      this.model.destroy();
    },

    _updateFeatureGroup: function () {
      if(cdb.god.featureGroups){
        for(var i = 0; i < cdb.god.featureGroups.length; i++){
          if(cdb.god.featureGroups[i].id === this.layerId){
            // this.map_leaflet.removeLayer(cdb.god.featureGroups[i].featureGroup);
            cdb.god.featureGroups.splice(i,1);
            break;
          }
        }
      }
