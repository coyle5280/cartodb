...
module.exports = BaseDialog.extend({
  ...
  _mergeAnnotations: function(mapImage, callback) {
    ...
    html2canvas($('.cartodb-map')[0], {
      allowTaint: true, // don't allow non cors images taint the canvas
      taintTest: false,
      // useCORS: true,
      proxy: {
        url: imageProxyURL,
      ...
      onclone: function(clonedDom) {
        ...
        doc.find('.cartodb-map > div:not(.annotation, .text, .image, .ExportImageView, .leaflet-control-container)').remove();
        doc.find('img').attr('crossOrigin', 'anonymous');
