...
module.exports = cdb.core.View.extend({
  ...
  _TEMPLATES: {
    ...
    regular: '<%- protocol %>:<%- mapsApiHost %>/user/<%- username  %>/api/v1/map/static/named/<%- tpl %>/<%- width %>/<%- height %>.png',
  ...
  _loadFromVisId: function() {
    ...
    var url_template = options.mapsApiHost + options.username + "/api/v1/map/static/named/" + options.tpl + "/" + options.width + "/" + options.height + ".png";
    var url = url_template.replace("{user}", "");
